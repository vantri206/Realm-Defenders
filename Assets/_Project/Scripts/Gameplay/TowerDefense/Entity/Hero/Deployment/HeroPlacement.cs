...
public class HeroPlacement : MonoBehaviour
{
    ...
    public bool CanPlaceHero(HeroCombatState combatState, CombatGridCell cell)
    {
        ...
        if (!combatState.Definition.IsValid)
        {
            return false;
        }
    ...
    public HeroRuntime PlaceHero(HeroCombatState combatState, CombatGridCell cell)
    {
        if (!CanPlaceHero(combatState, cell))
        {
            return null;
        }

        combatContext.CombatGrid.TryCellToWorldBottomCenter(cell, out Vector3 spawnPosition);
        HeroRuntime hero = Instantiate(combatState.Definition.Prefab, spawnPosition, Quaternion.identity, transform);

        hero.Initialize(combatState, combatContext, cell.CellPosition);

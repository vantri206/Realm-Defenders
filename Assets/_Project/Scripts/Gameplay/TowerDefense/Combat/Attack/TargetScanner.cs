...
public class TargetScanner : MonoBehaviour
{
    ...
    {
        if (combatGrid == null || combatGrid.Grid == null || owner == null || owner.BattleTeam == null)
        {
    ...
    public void Scan(Vector2 originPosition, IReadOnlyList<Vector2Int> pattern, TargetSide targetSide, AttackEffect attackEffect, UnitAttackType attackType, List<Hurtbox> results)
    {
        if (results == null)
        {
        ...
        for (int i = 0; i < pattern.Count; i++)
        {
            ...
            ScanAtPosition(scanPosition, targetSide, attackEffect, attackType, results);
        }
    }

    private bool IsValidTarget(Hurtbox hurtbox, TargetSide targetSide, AttackEffect attackEffect, UnitAttackType attackType)
    {
        if (hurtbox == null)
        {
            return false;
        ...
        if (!AttackTargetRulling.CanTarget(attackType, hurtbox))
        {
            return false;
        }
    ...
    private void ScanAtPosition(Vector2 centerPosition, TargetSide targetSide, AttackEffect attackEffect, UnitAttackType attackType, List<Hurtbox> results)
    {
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (!IsValidTarget(hurtbox, targetSide, attackEffect, attackType) || !uniqueTargets.Add(hurtbox))
            {
                continue;
            }
    ...
    {
        Vector2 offset = hurtbox.AimPosition - centerPosition;
